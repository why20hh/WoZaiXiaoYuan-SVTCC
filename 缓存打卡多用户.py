...
class Health:
    ...
    def doPunchIn(self):
        ...
        sign_data = {
            "answers": '["0"]',
            "latitude": "30.68800939192702",
            "longitude": "103.81650371398926",
            "country": "中国",
            "city": "成都市",
            "district": "温江区",
            "province": "四川省",
    ...
    def sendNotification(self):
        url = 'https://ts.xhyun.online/send_private_msg'
        notifyResult = self.getResult()
        dktime = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        msg = {
            'user_id': qq,
            'message':'健康打卡' + '打卡情况:' +notifyResult + '\n' +'打卡时间:' + dktime,

        }
        re = requests.post(url, params=msg)
        print(re.text)
...
if __name__ == "__main__":
    with open('/mnt/health/clock/shu.json', 'r', encoding='utf8')as fp:
        json_data = json.load(fp)
        for key in json_data:
            user = key['username']
            passwd = key['password']
            dldata = key['sign_data']
            cache = key['cache']
            qq = key['qq']
            print('当前打卡用户:' + user)
            main()
